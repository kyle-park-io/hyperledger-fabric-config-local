...
function deployChaincode() {
    ...
    for var in {0..0}; do
        ...
        echo ${PACKAGE_ID}
        set -x
        ${BIN_DIR}/peer lifecycle chaincode approveformyorg -o orderer0.example.com:7050 --ordererTLSHostnameOverride orderer0.example.com --tls --cafile $ORDERER_CA --channelID ${CHANNEL_NAME} --name ${TOKEN_NAME} --version ${CHAINCODE_VERSION} --package-id ${PACKAGE_ID} --sequence ${CHAINCODE_SEQUENCE} ${CHAINCODE_INIT_REQUIRED} ${CHAINCODE_END_POLICY} ${CHAINCODE_COLL_CONFIG} >&${LOG_PATH}
        res=$?
        { set +x; } 2>/dev/null
        cat ${LOG_PATH}
    ...
    for var in {0..0}; do
        ...
        set -x
        ${BIN_DIR}/peer lifecycle chaincode commit -o orderer0.example.com:7050 --ordererTLSHostnameOverride orderer0.example.com --tls --cafile $ORDERER_CA --channelID ${CHANNEL_NAME} --name ${TOKEN_NAME} $PEER_CONN_PARMS --version ${CHAINCODE_VERSION} --sequence ${CHAINCODE_SEQUENCE} ${CHAINCODE_INIT_REQUIRED} ${CHAINCODE_END_POLICY} ${CHAINCODE_COLL_CONFIG} >&${LOG_PATH}
        res=$?
        { set +x; } 2>/dev/null
        cat ${LOG_PATH}
